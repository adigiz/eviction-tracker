...
interface CaseDetailsModalProps {
  ...
  onClose: () => void;
  caseItem: LegalCase;
  onSubmit: (data: CaseDetailsFormData) => Promise<void>;
  formError: string;
  isSaving: boolean;
}
...
const CaseDetailsModal: React.FC<CaseDetailsModalProps> = ({
  ...
  const form = useZodForm(caseDetailsSchema, {
    defaultValues: {
      status: caseItem.status,
      paymentStatus: caseItem.paymentStatus,
      trialDate: caseItem.trialDate || "",
      districtCourtCaseNumber: caseItem.districtCourtCaseNumber || "",
      contractorId: caseItem.contractorId || "",
      courtOutcomeNotes: caseItem.courtOutcomeNotes || "",
    },
  });
  ...
  const handleSubmit = async (data: any) => {
    try {
      await onSubmit(data as CaseDetailsFormData);
      onClose();
    } catch (error) {
        ...
        <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <FormInput
              ...
              label="District Court Case Number"
              type="text"
            ...
            <div className="md:col-span-2">
              <FormInput
                ...
                label="Court Outcome Notes"
                type="text"
                rows={3}
                placeholder="Enter court outcome notes"
                isOptional
                disabled={form.formState.isSubmitting || isSaving}
              />
            </div>
