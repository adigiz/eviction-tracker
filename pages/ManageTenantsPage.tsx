      ...
      {tenants.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {tenants.map(tenant => {
            ...
            return (
              <div key={tenant.id} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-shadow duration-300 flex flex-col justify-between">
                <div className="p-6">
                  <h2 className="text-xl font-semibold text-primary-700 mb-2">
                    {displayTenantNamesArray(tenant.tenantNames)}
                  </h2>
                  {tenant.email && <p className="text-sm text-gray-600">Email: {tenant.email}</p>}
                  {tenant.phone && <p className="text-sm text-gray-600">Phone: {tenant.phone}</p>}
                  
                  {property && (
                    <div className="mt-3 pt-3 border-t border-gray-100">
                      <h3 className="text-xs font-semibold text-gray-500 uppercase mb-1">Associated Property</h3>
                      <p className="text-sm text-gray-600">{property.address}{property.unit ? `, ${property.unit}` : ''}</p>
                      <p className="text-sm text-gray-600">{property.city}, {property.state} {property.zipCode}</p>
                      {property.county && <p className="text-xs text-gray-500 mt-1">County: {property.county}</p>}
                      <p className="text-xs text-gray-500 mt-1">Type: {property.propertyType || 'N/A'}</p>
                      {property.description && <p className="text-sm text-gray-500 mt-2 italic break-words">Eviction Posting Info: {property.description}</p>}
                    </div>
                  )}
                  {!property && (
                    <div className="mt-3 pt-3 border-t border-gray-100">
                        <p className="text-sm text-red-500">Property details not found for this tenant.</p>
                    </div>
                  )}
                </div>
                <div className="bg-gray-50 p-4 flex justify-end space-x-2">
                  <button onClick={() => handleEditTenant(tenant)} 
                          className="text-xs font-medium text-primary-600 hover:text-primary-800 py-1 px-3 rounded-md hover:bg-primary-100 transition-colors">Edit</button>
                  <button onClick={() => handleDeleteTenant(tenant.id)}
                          className="text-xs font-medium text-red-600 hover:text-red-800 py-1 px-3 rounded-md hover:bg-red-100 transition-colors">Delete Tenant</button>
                </div>
              </div>
            );
          })}
